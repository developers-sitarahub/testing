  ...
  try {
      ...
      try {
        ...
        if (error.name === "TokenExpiredError") {
          return res.status(401).json({
            message:
              "Invite link has expired. Please contact the administrator for a new invitation.",
            code: "INVITE_EXPIRED",
          });
        }
        return res.status(401).json({
    ...
    if (isInviteFlow && user.activatedAt) {
      return res.status(400).json({
        message:
          "Account already activated. Invite link has been used. Please use 'Forgot Password' if you need to reset your password.",
  ...
  try {
    ...
    if (!user.activatedAt) {
      try {
        ...
        console.warn(
          "⚠️ Failed to emit user:activated event or log activity:",
          error.message,
        );
      }
    }
