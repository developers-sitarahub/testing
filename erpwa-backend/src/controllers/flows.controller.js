...
const deleteFlowTemplates = async (flowId, vendorId, accessToken, wabaId) => {
  try {
    ...
    const templates = await prisma.template.findMany({
      where: {
        vendorId,
        OR: [{ flowId: flowId }, { buttons: { some: { flowId: flowId } } }],
      },
    });
    ...
    for (const template of templates) {
      ...
      if (template.status !== "draft") {
        try {
          console.log(
            `🗑️ Deleting template "${template.metaTemplateName}" from Meta`,
          );
          ...
          console.warn(
            `⚠️ Failed to delete template "${template.metaTemplateName}" from Meta:`,
  ...
  try {
    ...
    try {
      ...
      if (vendor) {
        ...
        const errors = await flowsService.getValidationErrors(
          flow.metaFlowId,
          accessToken,
        );
    ...
    try {
      ...
      if (metaError.code === 100 || message.includes("validation")) {
        ...
        if (vendor && existingFlow) {
          const accessToken = decrypt(vendor.whatsappAccessToken);
          validationErrors = await flowsService.getValidationErrors(
            existingFlow.metaFlowId,
            accessToken,
          );
        }
  ...
  try {
    ...
    if (!vendor.whatsappFlowsPrivateKey) {
      ...
      try {
        ...
        console.log(
          "✅ Auto-setup Security Complete: Keys generated and uploaded.",
        );
      } catch (secError) {
    ...
    if (process.env.META_OAUTH_REDIRECT_URI) {
      try {
        ...
        console.warn(
          "⚠️ Could not resolve endpoint URI from .env:",
          uriErr.message,
        );
      }
    }
    ...
    if (finalEndpointUri) {
      ...
      try {
        ...
        console.log(
          `✅ Endpoint self-check successful: ${checkRes.status} ${checkRes.data}`,
        );
      } catch (checkErr) {
        console.warn(
          `⚠️ Endpoint self-check failed! Meta will likely reject this. URL: ${finalEndpointUri}. Error: ${checkErr.message}`,
        );
      ...
      try {
        ...
        console.warn(
          "Failed to sync endpoint_uri before publish:",
          metaUpdateErr.message,
        );
      }
    } else if (
      flow.flowJson &&
      /"version"\s*:\s*"[3-6]\.\d"/.test(
        typeof flow.flowJson === "string"
          ? flow.flowJson
          : JSON.stringify(flow.flowJson),
      )
    ) {
      ...
      return res.status(400).json({
        success: false,
        message:
          'Endpoint URI is missing. Please Edit the Flow and set the "Endpoint URI" before publishing.',
      });
    }
    ...
    try {
      ...
      console.error("Failed to get validation errors:", err);
    }

    // DIAGNOSTIC ERROR MESSAGE
    const envStatus = process.env.META_OAUTH_REDIRECT_URI
      ? "Loaded"
      : "Missing";
...
export const deleteFlow = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        "Warning: Failed to fully delete/deprecate Flow from Meta, but will remove from database:",
        error.message,
      );
    }
...
export const setupFlowsEncryption = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "WhatsApp not configured fully" });
    }
    ...
    res.json({
      success: true,
      message: "Flow encryption keys generated and uploaded successfully",
    });
  } catch (error) {
...
export const handleFlowEndpoint = async (req, res) => {
  try {
    ...
    if (
      req.method === "GET" ||
      (!req.body.encrypted_flow_data && !req.body.action)
    ) {
    ...
    if (!vendor.whatsappFlowsPrivateKey) {
      console.error(
        '❌ Vendor has no flow private key configured. Run "Setup Security" first.',
      );
      return res
        .status(400)
        .send("Flow encryption not configured. Please run Setup Security.");
    }
    ...
    try {
      ...
      console.error(
        "❌ Failed to decrypt stored private key:",
        keyError.message,
      );
    ...
    if (action === "ping") {
            ...
            if (flowId.length > 5) {
              ...
              if (flow && flow.flowJson) {
                ...
                if (json.screens && json.screens.length > 0) {
                  ...
                  console.log(
                    `✅ Dynamically resolved start screen: ${startScreenId} for Flow: ${flowId}`,
                  );
                }
              }
            }
      ...
      if (waId && vendor) {
        ...
        console.log(
          `📍 Current Screen: ${currentScreen} → Next Screen: ${nextScreen} [Status: ${status}]`,
        );
        ...
        try {
          ...
          if (lead) {
            ...
            if (status === "completed") {
              try {
                console.log(
                  "🎉 Flow Completed! Sending confirmation message...",
                );
                let confirmationText =
                  "Thank you! Your form response has been received.";
                ...
                if (flowId) {
                  ...
                  if (flowDetails) {
                    const niceCategory = flowDetails.category
                      ?.replace(/_/g, " ")
                      .toLowerCase();
                ...
                console.error(
                  "⚠️ Failed to send flow confirmation message:",
                  msgError.message,
                );
              }
            }
            ...
            console.warn(
              `⚠️ Lead not found for number: ${waId}. Flow data logged but not linked to lead.`,
            );
          }
        } catch (e) {
      ...
      try {
        ...
        console.error("Could not log validation error", e);
      }
