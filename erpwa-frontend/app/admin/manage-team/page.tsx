...
export default function ManageTeam() {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "sales" | "admin">(
    "all",
  );
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        ...
        console.warn("fetchUsers: Expected array but got:", res.data);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleActivated = (data: { userId: string; activatedAt: string }) => {
      setTeam((prevTeam) =>
        prevTeam.map((user) =>
          user.id === data.userId
            ? { ...user, activatedAt: data.activatedAt }
            : user,
        ),
      );
  ...
  const filteredTeam = team.filter((member) => {
    ...
    if (roleFilter === "sales") {
      ...
      matchesRole =
        member.role === "vendor_admin" || member.role === "vendor_owner";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && editingId) {
        ...
        setTeam((prev) =>
          prev.map((m) =>
            m.id === editingId ? { ...m, name: formData.name } : m,
          ),
        );
      } else {
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <select
                    value={roleFilter}
                    onChange={(e) =>
                      setRoleFilter(e.target.value as "all" | "sales" | "admin")
                    }
              ...
              <table className="w-full min-w-[640px]">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    <th className="text-left py-3 px-4 md:px-6 font-semibold text-foreground text-sm">
                      Name
                    </th>
                    <th className="text-left py-3 px-4 md:px-6 font-semibold text-foreground text-sm">
                      Email
          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-border">
              <h2 className="text-lg font-semibold text-foreground">
                {isEditMode ? "Edit Name" : "Add Team Member"}
              </h2>
              <button
