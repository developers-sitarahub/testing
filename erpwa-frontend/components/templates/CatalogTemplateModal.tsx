...
export default function CatalogTemplateModal({
  isOpen,
  onClose,
  onSubmit,
  initialData,
}: CatalogTemplateModalProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (
        initialData.templateType === "carousel" ||
        initialData.carouselCards?.length > 0 ||
        lang?.headerType === "CAROUSEL"
      ) {
        setTemplateType("carousel");
        if (
          initialData.carouselCards &&
          Array.isArray(initialData.carouselCards) &&
          initialData.carouselCards.length > 0
        ) {
          setCarouselCards(
            initialData.carouselCards.map((c: any) => ({
              title: c.title || "",
              subtitle: c.subtitle || "",
              image: null,
        ...
        initialData.templateType === "catalog" ||
        initialData.catalogProducts?.length > 0 ||
        lang?.headerType === "CATALOG"
      ) {
        setTemplateType("catalog");
        if (
          initialData.catalogProducts &&
          Array.isArray(initialData.catalogProducts) &&
          initialData.catalogProducts.length > 0
        ) {
          // Assuming catalogProducts in DB are objects { productId: string }
          setCatalogProducts(
            initialData.catalogProducts.map((p: any) => p.productId || ""),
          );
        }
      } else {
        setTemplateType("carousel");
      ...
      if (templateType === "carousel") {
        for (let i = 0; i < carouselCards.length; i++) {
          if (!carouselCards[i].title)
            throw new Error(`Card ${i + 1} requires a title`);
          if (!carouselCards[i].image && !carouselCards[i].imagePreview)
            throw new Error(`Card ${i + 1} requires an image`);
        }
            ...
            >
              ...
              <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-12 bg-muted/5 divide-y lg:divide-y-0 lg:divide-x divide-border/50">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea
                          className="w-full px-4 py-3 bg-background border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none min-h-30 resize-none"
                          ...
                          <div className="space-y-4">
                            <div className="p-4 bg-primary/5 border border-primary/20 rounded-xl space-y-2">
                              ...
                              <p className="text-[10px] text-muted-foreground leading-relaxed">
                                Enter product IDs from your Meta Catalog. Images
                                and details will auto-populate on the customer's
                                device.
                              </p>
                            </div>
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between">
                                <label className="text-xs font-semibold">
                                  Products (
                                  {
                                    catalogProducts.filter((p) => p.trim())
                                      .length
                                  }
                                  /30)
                                </label>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-65 overflow-y-auto pr-2 scrollbar-thin">
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                                Carousel Config ({carouselCards.length}/10)
                              </label>
                              <Button
                                variant="outline"
                                size="sm"
                ...
                >
                  ...
                  <div className="relative mx-auto w-full max-w-70 border-[10px] border-border rounded-[48px] shadow-2xl bg-card transition-all duration-500 overflow-hidden transform lg:scale-102 2xl:scale-105">
                      ...
                      <div className="relative z-10 w-full flex flex-col gap-1 mt-1 animate-in fade-in zoom-in-95 duration-500">
                        <div className="bg-card rounded-2xl rounded-tl-none shadow-lg relative overflow-hidden group border border-border">
                          <div className="px-3 pt-3 pb-3 text-[13px] leading-snug text-foreground/80 whitespace-pre-wrap font-sans">
                            {bodyText ||
                              "Design your message and it will appear here in real-time..."}
                          ...
                          {templateType === "catalog" &&
                            catalogProducts.filter((p) => p.trim()).length >
                              0 && (
                              <div className="px-3 pb-3 border-t border-border bg-muted/10">
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                  {catalogProducts
                                    ...
                                    .map((product, idx) => (
                                      <div
                                        key={idx}
                                        className="aspect-square bg-muted dark:bg-[#2c3941] rounded-xl relative overflow-hidden group border border-black/5 transition-all hover:scale-[1.02]"
                                      >
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-2 backdrop-blur-[2px] transition-all group-hover:bg-black/80">
                                          <div className="text-[9px] text-white truncate font-bold text-center tracking-wide">
                                            {product}
                                          </div>
                                        </div>
                                      </div>
                                ...
                                {catalogProducts.filter((p) => p.trim())
                                  .length > 4 && (
                                  <div className="mt-2 py-1 text-center text-[10px] font-bold text-primary bg-primary/10 rounded-lg">
                                    +{" "}
                                    {catalogProducts.filter((p) => p.trim())
                                      .length - 4}{" "}
                                    More Products
                          ...
                          <div className="relative mt-2 group/carousel">
                            ...
                            >
                              {carouselCards.map((card, idx) => (
                                <div
                                  key={idx}
                                  className="bg-card shrink-0 w-55 rounded-2xl shadow-lg overflow-hidden border border-border snap-center"
                                >
                                  ...
                                  <div className="p-3 space-y-1">
                                    <div className="text-[12px] font-bold text-foreground truncate">
                                      {card.title || `Product ${idx + 1}`}
                                    </div>
                                    <div className="text-[10px] text-muted-foreground line-clamp-2 leading-tight h-7">
                                      {card.subtitle ||
