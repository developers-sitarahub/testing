...
export function CoolTooltip({ content, children, className }: CoolTooltipProps) {
    ...
    return (
        <div className={`relative group ${className || ""}`}>
            {children}
            <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block z-50 w-max max-w-[200px] px-3 py-2 text-xs font-medium text-white bg-black/90 rounded-md shadow-xl backdrop-blur-sm dark:bg-slate-800 dark:text-white dark:border dark:border-slate-700 pointer-events-none whitespace-normal break-words transition-opacity duration-200">
                {content}
                {/* Arrow */}
